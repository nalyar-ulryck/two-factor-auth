...
trait InstallsApiStack
{
    ...
    protected function installApiStack()
    {
        ...
        // Controllers...
        $files->ensureDirectoryExists(app_path('Http/Controllers/TwoFactorController'));
        $files->copyDirectory(__DIR__.'/../../Http/Controllers/TwoFactorController', app_path('Http/Controllers/TwoFactorController'));

        // Middleware...
        $files->copyDirectory(__DIR__.'/../../Http/Middleware', app_path('Http/Middleware'));

        $this->installMiddlewareAliases([
            'twofactor' => '\App\Http\Middleware\TwoFactorAuthenticated::class',
        ]);

        $this->installMiddleware([
            '\Laravel\Sanctum\Http\Middleware\TwoFactorAuthenticated::class',
        ], 'api', 'prepend');
        ...
        // Configuration...
        $files->copyDirectory(__DIR__.'/../../config', config_path());
